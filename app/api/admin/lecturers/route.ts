...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ success: true, data: await listLecturers(query.search, query.page, query.pageSize) });
  } catch (error) {
    ...
    return NextResponse.json({ success: false, message: "Unable to load lecturers." }, { status: 500 });
  }
}
